...
def normalize_type(channel: Dict[str, Any]) -> str:
    ...
    if current == "RADYO" or (video_missing and audio_present) or has_radio_name_hint(channel.get("kanal_adi", "")):
        return "RADYO"
    return "TV"
...
def normalize_quality(channel: Dict[str, Any]) -> str:
    ...
    name = clean_text(channel.get("kanal_adi")).upper()
    if "4K" in name or "UHD" in name or current == "4K":
        return "4K"
    if "HD" in name or current == "HD":
        return "HD"
    return current or "SD"
...
def normalize_channel(channel: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    normalized["uydu"] = clean_text(channel.get("uydu") or channel.get("Uydu")) or "Türksat 42.0°E"
...
def normalize_dataset(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "uydu_adi": clean_text(data.get("uydu_adi")) or "Türksat 42.0°E",
    ...
    for channel in channels:
        ...
        tp = grouped.setdefault(
            tp_id,
            {
                ...
                "fec": channel.get("fec", ""),
                "uydu": channel.get("uydu", "Türksat 42.0°E"),
...
def validate(old_data: Optional[Dict[str, Any]], new_data: Dict[str, Any], diff: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
    ...
    technical_count = len(diff.get("technical_changes", []))
    if technical_count > MAX_TECHNICAL_CHANGES_WITHOUT_BLOCK:
        errors.append(f"Teknik değişiklik sayısı anormal yüksek: {technical_count}")
