...
FILTERS = {
    ...
    "RD": {"yayin": "RD", "kalite": "SD", "tur_hint": "RADYO"},
    # Genel tarama önemli; ancak burada artık kör şekilde TV demiyoruz.
    "Tümü": {"yayin": "", "kalite": "SD", "tur_hint": "UNKNOWN"},
}
...
def infer_quality(filter_key: str, default_quality: str, name: str) -> str:
...
def infer_type(filter_key: str, tur_hint: str, name: str, video_pid: str, audio_pid: str) -> str:
    ...
    # Yedek kural: Tümü filtresinde yakalanan ve adı radyo gibi olanları TV sayma.
    if tur_hint == "UNKNOWN" and has_radio_name_hint(name):
        return "RADYO"
    ...
    for channel in channels:
        ...
        tp = grouped.setdefault(
            tp_id,
            {
                ...
                "uydu": clean_text(channel.get("uydu")) or "Türksat 42.0°E",
...
def normalize_channel(raw: Dict[str, Any], filter_key: str, filter_settings: Dict[str, str]) -> Optional[Dict[str, Any]]:
    ...
    quality = infer_quality(filter_key, filter_settings.get("kalite", "SD"), name)
    ...
    return {
        ...
        "video_pid": video_pid,
        "ses_pid": audio_pid,
        "uydu": clean_text(raw.get("uydu")) or "Türksat 42.0°E",
        "kalite": quality,
        "tur": channel_type,
        "tp_id": tp_id,
    }
            ...
            try:
                ...
                for row in rows:
                    ...
                    if existing.get("tur") == "TV" and channel.get("tur") == "RADYO":
                        existing["tur"] = "RADYO"
                    if existing.get("kalite") == "SD" and channel.get("kalite") in {"HD", "4K"}:
