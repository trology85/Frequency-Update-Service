...
FILTERS = {
    "HD": {"yayin": "HD", "kalite": "HD", "tur_hint": "TV"},
    "SD": {"yayin": "SD", "kalite": "SD", "tur_hint": "TV"},
    "4K": {"yayin": "4K", "kalite": "4K", "tur_hint": "TV"},
    "RD": {"yayin": "RD", "kalite": "Bilinmeyen", "tur_hint": "RADYO"},
    # Genel tarama yalnız eksik kayıtları tamamlar; kalite bilinmiyorsa tahmin edilmez.
    "Tümü": {"yayin": "", "kalite": "Bilinmeyen", "tur_hint": "UNKNOWN"},
}
...
def quality_from_source_filter(filter_key: str, default_quality: str) -> str:
    """Use Türksat's selected broadcast-quality filter as the only quality source."""
    if filter_key in {"SD", "HD", "4K"}:
        return filter_key
    return default_quality if default_quality in {"SD", "HD", "4K"} else "Bilinmeyen"


def infer_type(filter_key: str, tur_hint: str, name: str, video_pid: str, audio_pid: str) -> str:
    if filter_key == "RD" or tur_hint == "RADYO":
        return "RADYO"

    video_missing = is_missing_pid(video_pid)
    audio_present = not is_missing_pid(audio_pid)
    if video_missing and audio_present:
        return "RADYO"

    # Kanal adı yayın türü için güvenilir veri değildir.
    return "TV"
...
def normalize_channel(raw: Dict[str, Any], filter_key: str, filter_settings: Dict[str, str]) -> Optional[Dict[str, Any]]:
    ...
    quality = quality_from_source_filter(filter_key, filter_settings.get("kalite", "Bilinmeyen"))
    channel_type = infer_type(filter_key, filter_settings.get("tur_hint", "UNKNOWN"), name, video_pid, audio_pid)
        ...
        while True:
            ...
            try:
                ...
                for row in rows:
                    ...
                    # Daha güvenilir filtre sonucu sonradan gelirse kalite/tür bilgisini iyileştir.
                    if existing.get("tur") == "TV" and channel.get("tur") == "RADYO":
                        existing["tur"] = "RADYO"
                    if existing.get("kalite") == "Bilinmeyen" and channel.get("kalite") in {"SD", "HD", "4K"}:
                        existing["kalite"] = channel["kalite"]
